...
local col = vim.fn.col('.') - 1 return col == 0 or vim.fn.getline('.'):sub(col, col):match('%s') ~= nil end
...
_G.tab_complete = function()
	if vim.fn.pumvisible() == 1 then return t "<C-n>" 
		elseif vim.fn('vsnip#available') == 1 then return t "<Plug>(vsnip-expand-or-jump)"
...
_G.s_tab_complete = function()
	if vim.fn.pumvisible() == 1 then return t "<C-p>"
		elseif vim.fn'vsnip#jumpable' == 1 then return t "<Plug>(vsnip-jump-prev)"
	end
end

-- If <S-Tab> is not working in your terminal, change it to <C-h> return t "<S-Tab>" end end
